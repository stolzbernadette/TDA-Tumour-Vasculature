function  [starts_dim0, ends_dim0, starts_dim1, ends_dim1] = getRadialFiltrationParallelRocheFullQuarterPoints(initial_path, folder, category, tumour, day, file_id, output_path, number_of_divisions)

    import edu.stanford.math.plex4.*;
    javaaddpath('./lib/javaplex.jar');
    
    % WE ONLY LOOK AT QUARTER THE FILTRATION FOR COMPUTATIONAL REASONS
    ...
    [VesselBranchingPoints,VesselNonBranchingPoints] = ...
        constructVesselPointCloudQuarterPoints(vessel_file_path);
    ...
    title(['Tumour ' char(tumour) ', Day ' char(day)], 'Interpreter', 'none','Fontsize',16)
  ...
  % We prepare the javaplex input
        ...
        if size(VesselPointCloud_point_indices_in_filtration,1) ~= size(VesselPointCloud,1)    
            ...
            if sorted_Distances_to_center(distance_vector_index) <= radius
                ...
                for point_number = 1:size(points_within_radius,1)
                    ...
                    direct_neighbours = getNeighbours_newQuarterPoints(point,Vessels,VesselBranchingPoints);
    ...
    options.filename = ['Radial filtration on Tumour ' char(tumour) ', Day ' char(day)];
    options.max_filtration_value = radius;
    options.max_dimension = 1;
    h3 = plot_barcodes_javaPlex(intervals, options);

    image_title2 = [output_path 'BarcodeRadial_Quarter.pdf'];
    saveas(h3,image_title2)

    image_title3 = [output_path 'BarcodeRadial_Quarter.fig'];
    ...
    fileID = fopen([output_path 'BarcodeDim0_Quarter.txt'],'w');
    fmt = '%d %d\n';
    fprintf(fileID,fmt, dim0');
    fclose(fileID);

    fileID = fopen([output_path 'BarcodeDim1_Quarter.txt'],'w');
    fmt = '%d %d\n';
    ...
    save([output_path 'Betti0_Quarter.mat'],'Betti0')
    save([output_path 'Betti1_Quarter.mat'],'Betti1')

    image_title6 = [output_path 'Betti0Curve_Quarter.pdf'];
    image_title7 = [output_path 'Betti0Curve_Quarter.fig'];

    image_title4 = [output_path 'Betti1Curve_Quarter.pdf'];
    image_title5 = [output_path 'Betti1Curve_Quarter.fig'];
