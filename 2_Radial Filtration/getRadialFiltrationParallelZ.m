function  [starts_dim0, ends_dim0, starts_dim1, ends_dim1] = getRadialFiltrationParallelZ(initial_path, folder, category, tumour, day, file_id, output_path, number_of_divisions)
    ...
    N_ZVessels = length(ZVessels);

    [VesselBranchingPoints,VesselNonBranchingPoints] = ...
        ZconstructVesselPointCloudAllPoints(vessel_file_path);
    ...
    image_title = [output_path 'ZCenters.pdf'];
    image_titlefig = [output_path 'ZCenters.fig'];
  ...
  % We prepare the javaplex input
        ...
        if size(VesselPointCloud_point_indices_in_filtration,1) ~= size(VesselPointCloud,1)    
            ...
            if sorted_Distances_to_center(distance_vector_index) <= radius
                ...
                for point_number = 1:size(points_within_radius,1)
                    ...
                    direct_neighbours = getNeighbours_new(point,ZVessels,VesselBranchingPoints);
    ...
    image_title2 = [output_path 'ZBarcodeRadial.pdf'];
    saveas(h3,image_title2)

    image_title3 = [output_path 'ZBarcodeRadial.fig'];
    ...
    fileID = fopen([output_path 'ZBarcodeDim0.txt'],'w');
    fmt = '%d %d\n';
    fprintf(fileID,fmt, dim0');
    fclose(fileID);

    fileID = fopen([output_path 'ZBarcodeDim1.txt'],'w');
    fmt = '%d %d\n';
    ...
    save([output_path 'ZBetti0.mat'],'Betti0')
    save([output_path 'ZBetti1.mat'],'Betti1')

    image_title6 = [output_path 'ZBetti0Curve.pdf'];
    image_title7 = [output_path 'ZBetti0Curve.fig'];

    image_title4 = [output_path 'ZBetti1Curve.pdf'];
    image_title5 = [output_path 'ZBetti1Curve.fig'];
